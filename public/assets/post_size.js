  ...
  for (let i = 0; i < messageArr.length; i += 1) {
    ...
    if (messageArr[i].includes('http://') || messageArr[i].includes('https://') || messageArr[i].includes('www.')) {
      ...
      if (messageArr[i].startsWith('http://') || messageArr[i].startsWith('https://') || messageArr[i].startsWith('www.')) {
        // проверяем заканчивается ли элемент массива (строка) на символ, допустимый в доменах верхнего уровня
        ...
        if ((lastChar >= 48 && lastChar <= 57) || (lastChar >= 65 && lastChar <= 90) || (lastChar >= 97 && lastChar <= 122)) {
          ...
          for (let z = 1; z <= messageArr[i].length; z += 1) {
            let lastChar = messageArr[i].charCodeAt(messageArr[i].length - z);
            if ((lastChar >= 48 && lastChar <= 57) || (lastChar >= 65 && lastChar <= 90) || (lastChar >= 97 && lastChar <= 122)) {
        ...
        if ((lastChar >= 48 && lastChar <= 57) || (lastChar >= 65 && lastChar <= 90) || (lastChar >= 97 && lastChar <= 122)) {
          ...
          for (let y = 1; y <= messageArr[i].length; y += 1) {
            let lastChar = messageArr[i].charCodeAt(messageArr[i].length - y);
            if ((lastChar >= 48 && lastChar <= 57) || (lastChar >= 65 && lastChar <= 90) || (lastChar >= 97 && lastChar <= 122)) {
              // удаляем ссылку из элемента массива, оставляем части строки до и после ссылки            
